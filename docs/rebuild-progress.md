...
- **Captura "sin período" box made ACTIONABLE (2026-06-13):** the old yellow note just said "Créelos en Planilla" (dead end — Jorge: *"It doesn't resolve the issue! It's useless!"*). Now: MASTER/ADMIN get an inline **"Extender período hasta {date}"** (PATCH the open period) / **"Crear período"** button that resolves the uncovered days in place and refreshes (typed cells preserved); FIELD gets a clear "ask an admin" message. `captura/page.tsx` passes `canManagePeriods`; `grid-client.tsx` `resolveUncovered()`.
- **Overlap bug found & fixed (2026-06-13):** the Extender button had pulled #8's start back to 05-11, overlapping closed #7 (04-13…05-14), because the captura grid only loaded OPEN periods → closed-period days looked "uncovered". Corrected boundaries to Jorge's actual cycle: **#7 closed 04-13…05-13 (paid thru 05-13), #8 open 05-14…06-13** (contiguous, no overlap, 0 records orphaned). Hardened so it can't recur:
  - **Captura grid loads ALL periods** (open+closed); closed-period days render **locked 🔒 / disabled** with a "período cerrado (histórico)" note — never treated as uncovered, never offered for extend. Save uses `openPeriodFor` and skips closed/uncovered days (server already rejected closed via `isClosed:false`).
  - **Server overlap guard** on `POST` + `PATCH /api/pay-periods`: reject any range overlapping another period (409). Period integrity guaranteed regardless of button or manual edit. (Distinct from the record-orphan freedom, which stays.)
- **Captura now PREFILLS saved records (2026-06-13):** the grid was write-only — it opened blank and never reloaded saved entries, so field users thought data was lost (it wasn't: verified 500 records / 39 workers written to #8 in 24h, all on `/planilla`). Fix: `captura/page.tsx` loads the open period's records and passes `existing`; `grid-client.tsx` seeds `cells` from them, unions record-holders into the roster, and opens on the **most recent week with data**. One record per (worker,day) so the grid maps losslessly. Closed-period cells stay locked/disabled.
...
- **Dedup page REMOVED (2026-06-13, workflow changed):** the clean SSOT roster has no duplicates ("Sin duplicados: 39"), so `/admin/trabajadores-duplicados` is retired. Deleted: the page (`page.tsx`+`dedup-client.tsx`), its only backend `POST /api/admin/workers/merge`, the dedup lib `src/lib/workers/duplicate-clusters.ts`, and the sidebar link (+`Merge` icon import). Worker mutation surface is now **2 endpoints** (create `/api/workers`, edit `/api/workers/[id]`); offboarding = deactivate via the worker profile (`isActive`). tsc clean (the only errors were stale `.next/types`, cleared).
