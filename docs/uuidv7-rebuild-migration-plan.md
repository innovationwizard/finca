...
- Recreate all 14 tables with **identical structure** to current, **except employees**, which gets the **enriched schema** (DEC-1):
  - `worker` (identity): `id` UUIDv7; `cui` (normalized, **unique** — identity key); structured `given_names`/`apellido_paterno`/`apellido_materno` (+ derived display name); `date_of_birth`, `sex`, `nationality`, birthplace, `comunidad_linguistica`, `estado_civil`, lifecycle `status`, `person_photo_url`; provenance (`extraction_confidence`, `notes`, `source_file`, `source_page`).
...
- **DEC-1a ✅ resolved:** shared `worker_document` parent + typed `dpi_document` / `birth_certificate_document` children (class-table inheritance). Two finer modeling points I'm **recommending, not assuming** — confirm or correct:
  - **Person-attribute placement ✅ resolved:** canonical values (`date_of_birth`, `sex`, birthplace, `estado_civil`, etc.) live on the `worker` identity (golden record), **and** each document keeps an as-stated copy for provenance (`dpi_document` / `birth_certificate_document`).
