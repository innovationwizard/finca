...
// live (TOTAL − descuento + adicional). Saves only changed rows to
// PATCH /api/planilla/ajustes; read-only roles see disabled inputs.
// =============================================================================
...
type Row = { workerId: string; name: string; gross: number; deductions: number; bonification: number };
type Draft = { deductions: string; bonification: string };
...
export function AjustesGrid({ periodId, rows, canWrite }: { periodId: string; rows: Row[]; canWrite: boolean }) {
  ...
  // Rows whose draft differs from the server value.
  const changed = useMemo(
    () =>
      rows.filter((r) => {
        const d = drafts[r.workerId];
        return n(d.deductions) !== r.deductions || n(d.bonification) !== r.bonification;
      }),
    [rows, drafts],
  );
    ...
    try {
      const res = await fetch("/api/planilla/ajustes", {
        ...
        body: JSON.stringify({
          ...
          rows: changed.map((r) => ({ workerId: r.workerId, deductions: n(drafts[r.workerId].deductions), bonification: n(drafts[r.workerId].bonification) })),
        }),
      });
  ...
  }, [changed, drafts, periodId, router]);
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-finca-100 bg-finca-50/50 text-finca-600">
              ...
              <th className="px-3 py-2.5 text-right font-medium">Descuentos</th>
              <th className="px-3 py-2.5 text-right font-medium">Adicionales</th>
            ...
            {visible.map((r, idx) => {
              ...
              return (
                <tr key={r.workerId} className="hover:bg-finca-50/30">
                  <td className="px-2 py-1.5 text-finca-400 tabular-nums">{idx + 1}</td>
                  <td className="whitespace-nowrap px-3 py-1.5 font-medium text-finca-900">{r.name}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums text-finca-700">{formatGTQ(r.gross)}</td>
                  <td className="px-3 py-1.5 text-right">
                    <input
                      ...
                      placeholder="0.00"
                      className="w-24 rounded border border-finca-200 px-2 py-1 text-right text-sm tabular-nums focus:border-earth-400 focus:outline-none disabled:cursor-not-allowed disabled:bg-finca-50 disabled:text-finca-400"
                    />
                  </td>
                  <td className="px-3 py-1.5 text-right">
                    <input
                  ...
                  <td className="px-3 py-1.5 text-right tabular-nums font-semibold text-finca-900">{formatGTQ(pay)}</td>
                </tr>
              );
            })}
            {visible.length === 0 && (
